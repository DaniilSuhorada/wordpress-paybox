...
class WC_Paybox_Payment_Gateway extends WC_Payment_Gateway
{
    ...
    public function handle_itn_payment_complete($data, $order, $subscriptions)
    {
        ...
        if ($this->order_contains_pre_order($order_id)
            ...
            if (!$is_pre_order_fee_paid) {
                ...
                $order->add_order_note(
                    sprintf(
                        __('PayBox pre-order product line total paid: R %1$s (%2$s)', 'paybox-payment-gateway'),
                        $data['amount_gross'],
                        $data['pf_payment_id']
                    )
                );
    ...
    public function scheduled_subscription_payment($amount_to_charge, $renewal_order)
    {
        ...
        if (is_wp_error($response)) {
            ...
            $renewal_order->update_status(
                $this->get_option('failure_status'),
                sprintf(
                    __('PayBox Subscription renewal transaction failed (%1$s:%2$s)', 'paybox-payment-gateway'),
    ...
    public function setup_constants()
    {
        ...
        define('PF_ERR_UNKNOWN', __('Unknown error occurred', 'paybox-payment-gateway'));
