...
class WC_Paybox_Payment_Gateway extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->method_description = sprintf(
            __(
                'PayBox works by sending the user to %PayBox%2$s to enter their payment information.',
                'paybox-payment-gateway'
            ),
            '<a href="https://paybox.money">',
            '</a>'
        );
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'language'        => array(
                'title'       => __('Payment page language', 'paybox-payment-gateway'),
                'type'        => 'select',
                'description' => __('Place the payment gateway in development mode.', 'paybox-payment-gateway'),
                'default'     => 'ru',
                'options'     => array(
            ...
            'ofd_in_delivery' => array(
                'title'       => __('Delivery in OFD', 'paybox-payment-gateway'),
                'type'        => 'checkbox',
    ...
    public function admin_options()
    {
        if (in_array(get_woocommerce_currency(), $this->available_currencies)) {
            ...
            <h3><?php
                echo esc_attr(
                    (!empty($this->method_title)) ? $this->method_title : __('Settings', 'paybox-payment-gateway')
                ); ?></h3>
    ...
    public function generate_Paybox_form($order_id)
    {
        ...
        return '<form action="' . esc_url($this->url) . '" method="post" id="Paybox_payment_form">
                ...
                <input type="submit" class="button-alt" id="submit_Paybox_payment_form" value="' . __(
                'Pay via PayBox',
                'paybox-payment-gateway'
                ...
                <script type="text/javascript">
                    jQuery(function(){
                        jQuery("body").block(
                            {
                                message: "' . __(
                'Thank you for your order. We are now redirecting you to PayBox to make payment.',
                'paybox-payment-gateway'
            ) . '",
                                overlayCSS:
                                {
    ...
    public function setup_constants()
    {
        ...
        define('PPWC_ERR_CONNECT_FAILED', __('Failed to connect to PayBox', 'paybox-payment-gateway'));
        define('PPWC_ERR_INVALID_SIGNATURE', __('Security signature mismatch', 'paybox-payment-gateway'));
        define('PPWC_ERR_MERCHANT_ID_MISMATCH', __('Merchant ID mismatch', 'paybox-payment-gateway'));
        define('PPWC_ERR_NO_SESSION', __('No saved session found for ITN transaction', 'paybox-payment-gateway'));
        define('PPWC_ERR_ORDER_ID_MISSING_URL', __('Order ID not present in URL', 'paybox-payment-gateway'));
        ...
        define('PPWC_ERR_ORDER_NUMBER_MISMATCH', __('Order Number mismatch', 'paybox-payment-gateway'));
        define('PPWC_ERR_ORDER_PROCESSED', __('This order has already been processed', 'paybox-payment-gateway'));
        define('PPWC_ERR_PDT_FAIL', __('PDT query failed', 'paybox-payment-gateway'));
        define('PPWC_ERR_PDT_TOKEN_MISSING', __('PDT token not present in URL', 'paybox-payment-gateway'));
